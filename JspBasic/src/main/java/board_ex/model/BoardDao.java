...
//	
	...
	*/
	public int insert( BoardVO rec ) throws BoardException
	{
		...
		ResultSet rs = null;
		Statement stmt	= null; //SQL쿼리를 실행하는 객체 
		...
		try{
			...
			//return insertedCount; //몇개 저장했나?
			return newSeq; //새로생성한 글 번호는 무엇인지
	...
	*/
	...
	public List<BoardVO> selectList(int startRow, int endRow) throws BoardException {
		...
		try{
			...
			// * sql 문장만들기
			String sql = "SELECT seq, title, writer, regdate, content, cnt "
					+ "	FROM board_ex "
